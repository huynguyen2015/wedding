...
import CurrencyInputV1 from '../index'

describe('Test for Currency Input Component', function () {
  let wrapper
  ...
  it('Test output with scientific notation 1e-8 === 0.00000001', async () => {
    wrapper = mount(<CurrencyInputV1 value="1e-8" />)
  ...
  it('Test number was formatted', async () => {
    wrapper = mount(<CurrencyInputV1 value="100000" />)
    const input = wrapper.find('input')
    expect(input.length).toBe(1)
  ...
  it('Test input change value and emmit to parent', async () => {
    let value = '1e-8'
    wrapper = shallow(
      <CurrencyInputV1 value={value} onChange={(e) => (value = e)} />
    )
      .dive()
      .dive()
      .dive()
